...
しかし、一部のバックエンドアプリに`https_redirection = true`を設定したい場合、`rpxy`はデフォルトでポートBでリダイレクションレスポンス301を発行しますが、これはコンテナ外からアクセスできません。

この問題を回避するため、`config.toml`で`public_https_port`（v0.13.0より前は`https_redirection_port`という名前でした）を指定してリダイレクションレスポンス用のカスタムポートを設定できます。この場合、`public_https_port`にポートAを設定すると、リダイレクションレスポンス301がポートAで発行されます。このポートはHTTP/3の`Alt-Svc`広告にも使われます。
...
public_https_port = 443   # `rpxy`はポート443でリダイレクションレスポンス301を発行
```
